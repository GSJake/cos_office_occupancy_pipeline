...
- YTD Avg. Attendance Count (avg daily in-office attendance on hybrid-flagged days, fallback to weekdays if missing)
...
def main() -> int:
    ...
    # Hybrid-day slice for occupancy metrics (fallback to weekdays per office when needed)
    if 'is_hybrid_day' in df_ytd.columns:
        df_hybrid = df_ytd[df_ytd['is_hybrid_day'] == True].copy()
        if df_hybrid.empty:
            df_for_avg = df_ytd.copy()
        else:
            hybrid_offices = set(df_hybrid['office_location'])
            all_offices = set(df_ytd['office_location'])
            fallback_offices = all_offices - hybrid_offices
            if fallback_offices:
                df_fallback = df_ytd[df_ytd['office_location'].isin(fallback_offices)].copy()
                df_for_avg = pd.concat([df_hybrid, df_fallback], ignore_index=True)
            else:
                df_for_avg = df_hybrid
    else:
        df_for_avg = df_ytd.copy()

    # YTD Avg. Attendance Count per office (sum LOB per day on hybrid days, then mean over days)
    daily_office_totals = (
        df_for_avg.groupby(['office_location', 'date'])['attendance_count']
        .sum()
